...
describe('antwerpen', () => {
    ...
    describe('createService', () => {
        it('should query CRAB if detecting a number', () => {
            ...
            const createService = proxyquire('../dist/antwerpen/service', {
        ...
        it('should query SOLR for a streetname or poi', () => {
            ...
            const createService = proxyquire('../dist/antwerpen/service', {
                'request': (options, handler) => {
                    expect(options.url).toEqual(
                        solrTestUrl + "?wt=json&rows=5&solrtype=gislocaties&dismax=true&bq=exactName:DISTRICT^20000.0&bq=layer:straatnaam^20000.0&q=(generaal%20armstrongweg)");
                    return handler(null, {statusCode: 200}, dummySolrResult);
                }
            });
            const fn = createService({
                solrGisAuthorization: solrTestAuth,
    ...
    describe('createController', () => {
        it('should call the service and output json', (done) => {
            const testResult = {};
            const createController = proxyquire('../dist/antwerpen/controller', {
                './service': () => (search) => {
